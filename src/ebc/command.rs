...
impl CommandSender {
    async fn do_send(&self, cmd: Command, ctx: &String) -> anyhow::Result<()> {
        self.0.send(cmd).await.with_context(|| format!("Failed to send {ctx}"))
    }
    ...
    pub async fn with_reply<T>(&self, cmd: impl Into<Command>, reply: oneshot::Receiver<T>) -> anyhow::Result<T> {
        let cmd = cmd.into();
        let ctx_str = cmd.get_command_str();

        self.do_send(cmd, &ctx_str).await?;
        reply.await.with_context(|| "Failed to get reply from {ctx_str}")
    }
}
