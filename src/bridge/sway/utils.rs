...
pub(super) fn get_all_windows_and_app(workspace: &Node, transform: &Matrix3<f64>) -> (HashSet<pid_t>,Vec<SwayWindow>) {
...
pub(super) fn output_to_transform(output: &Output) -> Result<Matrix3<f64>> {
    let scale = output.scale.ok_or(anyhow!("Could not get output scale"))
        .and_then(|s| if s == -1.0 {
    ...
    let SwayRect { x, y, width, height, .. } = output.rect;

    let rel_to_abs = Translation2::new(-x as f64, -y as f64);

    let iso = match output.transform.as_deref().ok_or(anyhow!("Bad transform"))? {
        "normal" => {
            nalgebra::Isometry2::identity()
        },
        "90" => {
            nalgebra::Isometry2::new(Vector2::new(height as f64, 0.0), 90_f64.to_radians())
        },
        "180" => {
            nalgebra::Isometry2::new(Vector2::new(width as f64, height as f64), 180_f64.to_radians())
        },
        "270" => {
            nalgebra::Isometry2::new(Vector2::new(0f64, width as f64), 270_f64.to_radians())
        }
        _ => { bail!("Unsupported transform") }
    };
